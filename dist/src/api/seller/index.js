...
var _seller = require('src/lib/seller');

var _seller2 = _interopRequireDefault(_seller);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var sellers = new _seller2.default();
...
router.post('/', function (req, res) {
  ...
  if (areFieldsRight) {
    sellers.addSellerByEmail(body).then(function (response) {
      res.json({
        response: response
      });
    }).catch(function (e) {
...
router.post('/bulk', function (req, res) {
  sellers.createBulk().then(function (response) {
    res.json(response);
  }).catch(function (e) {
    res.status(_g.constants.ERROR_CODES.HTTP_STATUS.FORBIDDEN).send({
      success: false,
